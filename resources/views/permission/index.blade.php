...
@section('content')
    ...
    @include('permission._view')
    {{-- add new Permission modal start --}}
    @include('permission._create')
    {{-- edit Permission modal start --}}
    @include('permission._edit')
    {{-- edit Permission modal start --}}
    @include('common._audit', ['entidad' => 'permission'])
          ...
          <div class="px-1 col-lg-12">
            <div class="card shadow border-dark">
              <div class="card-header d-flex justify-content-between align-items-center">
                <div>
                    <h3 class="text-secondary mb-0" style="line-height: 1;">Permission</h3>
                    <span class="text-muted card-text fs-rem-80">Gestión de Permission</span>
                </div>
                <div>
                    <button class="btn btn-outline-secondary rounded-0" data-bs-toggle="modal" data-bs-target="#addPermissionModal"><i class="fas fa-plus me-sm-2"></i><div class="d-none d-sm-inline">Nuevo {{ __('Permission') }}</div></button>
                    <button class="btn btn-outline-secondary rounded-0" id="#report_permission_btn"><i class="fas fa-print me-sm-2"></i><div class="d-none d-sm-inline">Reporte</div></button>
                </div>
              </div>
              <div class="card-body px-2 p-lg-5" id="show_all_permission">
        ...
        $(function() {
            ...
            $("#add_permission_form").submit(function(e) {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    beforeSend: function() {
                        $("#add_permission_btn").prop('disabled',true);
                    },
                    success: function(r) {
                        if (r.status == 200) {
                    ...
                    complete: function() {
                        $(`#add_permission_btn`).prop("disabled",false);
                    },
                });
            });
            ...
            $("#edit_permission_form").submit(function(e) {
                e.preventDefault();
                let id = $('#edit_permission_form #id').val();
                const fd = new FormData(this);
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    beforeSend: function() {
                        $("#edit_permission_btn").prop('disabled',true);
                    },
                    success: function(r) {
                        if (r.status == 200) {
                            jshelper.success(r.messages);
                    ...
                    complete: function() {
                        $(`#edit_permission_btn`).prop("disabled",false);
                    },
                });
            });
                        ...
                        success: function(r) {
                            ...
                            if(r.status == 200){
                                r.audits.forEach((audit, i) => {
                                    ...
                                    $('<tr>')
                                        .html(
                                            `<td data-bs-toggle="collapse" href="#collapseRow${i}" permission="button" aria-expanded="false" aria-controls="collapseRow${i}">
                                ...
                                $(`#modal-permission-audits-label`).html("Auditoria");
                                $('#modal-permission-audits').modal('show');
                            } else {
                                console.log(r);
                                jshelper.failure();
                            }
